...
optimize.model.parameters = function(par=par,data=data,cost=cost.function,
                                     model="SEQ1.3",
                                     method="GenSA",
                                     lower=-Inf,
                                     upper=Inf,
  ...
  if (method == "GenSA"){
    ...
    optim.par = GenSA(par=par,fn=cost,
                      data=data,
                      lower=lower,
                      upper=upper,
  ...
  # might be buggy and return stupid results
  if (method %in% c("Nelder-Mead", "BFGS", "CG", "L-BFGS-B", "SANN","Brent")){
    # optimize model parameters using the 
    # default optim() routine (might not converge)
    optim.par = optim(par=par,fn=cost,
                      data=data,
                      method=method,
                      lower=lower,
                      upper=upper,
